...
if( !class_exists( '_82lottery_Woo_Listing_Sorter' ) ) {

    class _82lottery_Woo_Listing_Sorter {
        ...
        function __construct() {
            ...
            /* Sorter CSS */
                add_filter( 'lottery_82_app_woo_archive_css', array( $this, 'woo_sorter_css'), 10, 1 );

            /* Sorter js */
                add_filter( 'lottery_82_app_woo_archive_js', array( $this, 'woo_sorter_js'), 10, 1 );
        ...
        */
            ...
            function module_dir_path() {
                ...
                if( lottery_82_app_is_file_in_theme( __FILE__ ) ) {
                    return LOTTERY_82_MODULE_DIR . '/woocommerce/listings/sorter/';
                } else {
            ...
            function module_dir_url() {
                ...
                if( lottery_82_app_is_file_in_theme( __FILE__ ) ) {
                    return LOTTERY_82_MODULE_URI . '/woocommerce/listings/sorter/';
                } else {
        ...
        */
            function woo_sorter_settings() {
                ...
                $settings = apply_filters( 'lottery_82_app_woo_sorter_settings', array () );
        ...
        */
                ...
                if( is_shop() || is_product_category() || is_product_tag() ) {
                    ...
                    echo '<div class="product-layout-controller '.esc_attr($grid_controller_class).'">';
                        echo '<ul class="product-change-layout">';
                            echo '<li class="hidden"><span data-column="1" class="'.esc_attr($one_column_class).'">'.esc_html__('1', 'lottery_82_app').'</span></li>';
                            echo '<li><span data-column="2" class="'.esc_attr($two_column_class).'">'.esc_html__('2', 'lottery_82_app').'</span></li>';
                            echo '<li><span data-column="3" class="'.esc_attr($three_column_class).'">'.esc_html__('3', 'lottery_82_app').'</span></li>';
                            echo '<li><span data-column="4" class="'.esc_attr($four_column_class).'">'.esc_html__('4', 'lottery_82_app').'</span></li>';
                        echo '</ul>';
                    echo '</div>';
                    ...
                    echo '<div class="product-list-options-controller '.esc_attr($list_controller_class).'">';
                        echo '<ul class="product-list-options">';
                            echo '<li><span data-list-option="left-thumb" class="'.esc_attr($left_thumb_class).'">'.esc_html__('Left Thumb', 'lottery_82_app').'</span></li>';
                            echo '<li><span data-list-option="right-thumb" class="'.esc_attr($right_thumb_class).'">'.esc_html__('Right Thumb', 'lottery_82_app').'</span></li>';
                        echo '</ul>';
                    echo '</div>';
                ...
                if( is_shop() || is_product_category() || is_product_tag() ) {
                    ...
                    echo '<div class="product-display-controller">';
                        echo '<ul class="product-change-display">';
                            echo '<li><span data-display="grid" class="'.esc_attr($grid_display_mode_class).'">'.esc_html__('Grid', 'lottery_82_app').'</span></li>';
                            echo '<li><span data-display="list" class="'.esc_attr($list_display_mode_class).'">'.esc_html__('List', 'lottery_82_app').'</span></li>';
                        echo '</ul>';
                    echo '</div>';
            ...
            function woocommerce_pagination_args( $output ){
                ...
                $output = array(
                    'prev_text' => '<span>'.esc_html__('Prev', 'lottery_82_app').'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    ...
                    'next_text' => '<span>'.esc_html__('Next', 'lottery_82_app').'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
            ...
            function woo_filters_widget_area() {
                echo '<div class="product-loop-filters-area-group">';
                    echo '<div class="product-loop-filters-area-title">'.esc_html__('Filters', 'lottery_82_app').'</div>';
                echo '</div>';
            }
            ...
            function woo_sorting_custom_extension() {
                echo '<div class="product-loop-filters-area-content hide">';
                    echo '<div class="product-loop-filters-area-content-inner">';
                        dynamic_sidebar( 'lottery_82_app-shop-filters' );
                    echo '</div>';
                echo '</div>';
            }
...
if( !function_exists('lottery_82_app_woo_listing_sorter') ) {
	function lottery_82_app_woo_listing_sorter() {
		return _82lottery_Woo_Listing_Sorter::instance();
	}
}

lottery_82_app_woo_listing_sorter();