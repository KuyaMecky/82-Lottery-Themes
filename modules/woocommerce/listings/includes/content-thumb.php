...
if( ! function_exists( 'lottery_82_app_shop_woo_loop_product_thumbnail' ) ) {

	function lottery_82_app_shop_woo_loop_product_thumbnail() {
		...
		echo '<div class="product-thumb">';
				...
				if( $product->is_featured() ) {
					echo '<div class="featured-tag">
									<div>
										...
										<span>'.esc_html__('Featured', 'lottery_82_app').'</span>
									</div>
								</div>';
				}
			...
			echo '<a class="image" href="'.esc_url($product->get_permalink()).'" title="'.esc_attr($product->get_name()).'">';
				...
				if($product_hover_style != 'product-hover-egrpovrcnt') {
					echo '<div class="product-thumb-overlay" '.lottery_82_app_html_output($product_overlay_bgcolor).'></div>';
				}
				...
				if($product_show_labels) {
					...
					echo '<div class="product-labels">';
						...
						if( $product->is_on_sale() && $product->is_in_stock() ) {
							echo '<span class="onsale"><span>'.esc_html__('Sale', 'lottery_82_app').'</span></span>';
						} else if( !$product->is_in_stock() ) {
							echo '<span class="out-of-stock"><span>'.esc_html__('Sold Out','lottery_82_app').'</span></span>';
						}
						...
						if(isset($settings['product-new-label']) && $settings['product-new-label'] == 'true') {
							echo '<span class="new"><span>'.esc_html__('New', 'lottery_82_app').'</span></span>';
						}
				...
				do_action( 'lottery_82_app_woo_before_product_thumb_image', $product_id );
				...
				if($display_bg_image) {
					...
					echo '<div class="primary-image"><img src="'.esc_url($primary_image_src).'" alt="'.esc_attr__('Shop Primary Image', 'lottery_82_app').'" title="'.esc_attr__('Shop Primary Image', 'lottery_82_app').'" /></div>';
				}
				...
				if($secondary_image_src != '') {
					if($display_bg_image) {
						...
						echo '<div class="secondary-image"><img src="'.esc_url($secondary_image_src).'" alt="'.esc_attr__('Shop Secondary Image', 'lottery_82_app').'" title="'.esc_attr__('Shop Secondary Image', 'lottery_82_app').'" /></div>';
					}
				}
			...
			if($product_thumb_content) {
				...
				lottery_82_app_shop_woo_loop_product_thumb_content_setup($product_thumb_content);

				echo '<div class="product-thumb-content" '.lottery_82_app_html_output($product_thumb_content_overlay_bgcolor).'>';
					do_action('lottery_82_app_woo_loop_product_thumb_content', 'thumb');
					remove_all_actions('lottery_82_app_woo_loop_product_thumb_content');
				echo '</div>';
	...
	add_action( 'woocommerce_before_shop_loop_item_title', 'lottery_82_app_shop_woo_loop_product_thumbnail', 10 );
