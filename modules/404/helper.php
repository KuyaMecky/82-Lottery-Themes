<?php
add_action( 'lottery_82_app_after_main_css', 'notfound_style' );
function notfound_style() {
    if( is_404() ) {
        wp_enqueue_style( 'lottery_82_app-404', get_theme_file_uri('/modules/404/assets/css/404.css'), false, LOTTERY_82_THEME_VERSION, 'all');
    }
}

add_filter( 'lottery_82_app_add_inline_style', 'notfound_inline_style' );
function notfound_inline_style( $style ) {
    if( is_404() ) {
        $params = lottery_82_app_404_page_params();
...
function lottery_82_app_404_page_params() {
    $params = array(
        'enable_404message' => 1,
    ...
    return apply_filters( 'lottery_82_app_404_page_params', $params );
}